...
export default function CreatePrice({ tankCategories, customerTypes }) {
  const [form] = Form.useForm();

  const handleFinish = (values) => {
    Inertia.post(route(routes.PRICES_STORE), values);
  };
        ...
        <Row gutter={[0, 32]}>
          <Breadcrumbs
            breadcrumbs={[
              ...
              { name: "Buat Harga Pengisian Baru" },
            ]}
          />
          <Col span={24}>
            <Card className="da-border-color-black-40">
              <Row gutter={[0, 32]}>
                <Col span={24}>
                  <h4>Buat Harga Pengisian Baru</h4>
